...
- [x] 1.1.4 Generate NSManagedObject subclasses for User and AuditLogEntry (auto-generated by Xcode)
...
- [x] 3.1.3 Implement `log(event:userId:details:)` method to create AuditLogEntry
...
- [x] 5.1.1 Create `Core/Services/AuthenticationService.swift` (12,231 bytes)
- [x] 5.1.2 Inject `UserRepository`, `KeychainManager`, `BiometricAuthManager`, and `AuditLogger`
- [x] 5.1.3 Implement `register(email:password:passcode:fullName:authMethod:) async throws -> User`
- [x] 5.1.4 Implement `login(email:credential:authMethod:biometric:) async throws -> User`
- [x] 5.1.5 Implement `logout() async throws`
- [x] 5.1.6 Implement `createSession(for user: User) throws -> UUID` (session token)
- [x] 5.1.7 Implement `validateSession() -> User?` (check keychain for valid session)
- [x] 5.1.8 Implement `invalidateSession() throws`
- [x] 5.1.9 Add session timeout tracking (5-minute inactivity timer)
- [x] 5.1.10 Store user's preferred authMethod in keychain for login screen pre-population
- [x] 5.1.11 Write unit tests for authentication flows (IntegrationTests.swift - 11 tests)

### 5.2 Add Session Timeout Handling
- [x] 5.2.1 Create session timeout timer using `Combine` or `Timer`
- [x] 5.2.2 Reset timer on user interaction (tap, swipe, keyboard input)
- [x] 5.2.3 Invalidate session and navigate to login after 5 minutes inactivity
- [x] 5.2.4 Handle app backgrounding: start background timer, require re-auth if timeout exceeded
- [x] 5.2.5 Log session timeout events in audit log
...
- [x] 7.1.4 Implement `signUp()` method with async/await (handles both password and passcode paths)
...
- [x] 7.1.11 Implement `login()` method with biometric/password/passcode flow
